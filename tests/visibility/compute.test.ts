...
describe('metric visibility, spec 0.2', () => {
  it.each([[7, 157 / 162, 2], [9, 19 / 36, 51]])('single wall h=%s has hand-calculated ratio %s', (height, expected, blocked) => {
    ...
    try {
      ...
      expect(r.summary.all.total_weight).toBeCloseTo(216, 12);
  ...
  it('uses nearest candidate boundary and excludes the whole candidate from occlusion', () => {
    ...
    const target = r.samples.find(p => p.id === 'ring:90:30')!.target!;
  ...
  it('pushes indoor/boundary samples outward up to 30m, retaining original weights', () => {
    const s = scene(); s.buildings = [building('wall', rectangle(29, -1, 65, 1), 100)];
    const r = computeVisibility(s);
    const a = r.samples.find(p => p.id === 'ring:90:30')!;
    const b = r.samples.find(p => p.id === 'ring:90:60')!;
    expect(a.status).toBe('excluded'); // 35m needed.
    ...
    expect(r.summary.all.excluded).toBe(1); expect(r.summary.all.moved).toBe(1);
    expect(r.summary.all.excluded_ratio).toBeCloseTo(1 / 108);
    expect(r.summary.all.total_weight).toBeCloseTo(216 - 100 / 30);
  });
  it('generates 108 rings, 20 samples per station and 10 per school with independent weights', () => {
    ...
    const r = computeVisibility(s);
    expect(r.summary.station).toMatchObject({ generated: 40, visible: 40, total_weight: 120 });
  ...
  it('distinguishes unavailable sources from confirmed absence and no valid samples', () => {
    ...
    expect(computeVisibility(full)).toMatchObject({ status: 'missing', reason: 'no_valid_samples', summary: { all: { excluded: 108 } } });
  });
