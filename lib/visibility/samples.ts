...
export function generateSamples(scene: VisibilityScene): VisibilitySample[] {
  ...
  for (let bearing = 0; bearing < 360; bearing += 10) for (const radius of [30, 60, 100]) {
  ...
  const path = (anchor: Anchor, group: 'station' | 'school', n: number, weight: number) => {
    for (let i = 1; i <= n; i++) samples.push({ id: `${group}:${anchor.id}:${i}`, group, anchor_id: anchor.id,
      step: i, point: at(scene.candidate, anchor.point, i / n), weight });
  };
  for (const station of scene.stations) path(station, 'station', 20, 3);
  for (const school of scene.schools) path(school, 'school', 10, 2);
  return samples;
}
