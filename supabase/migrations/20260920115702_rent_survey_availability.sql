-- Preserve official aggregate facts even when no verified spatial definition is available.
-- Such rows must remain unreachable through candidate lookup.
alter table public.rent_areas alter column geom drop not null;
alter table public.rent_areas add constraint verified_rent_area_geometry
  check (not mapping_verified or geom is not null);

-- A single rent_level always describes one source row. Missing rent falls back;
-- missing vacancy stays NULL at that row rather than silently mixing spatial levels.
