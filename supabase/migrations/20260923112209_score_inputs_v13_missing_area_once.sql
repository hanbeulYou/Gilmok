-- A fresh address payload already emits this null path. Preserve one reason per path.
...
begin
 ...
 -- Add authoritative register gross area to both footprint and address paths.
  lookup_started:=clock_timestamp();
  b:=r->'building';
  if b is not null and b<>'null'::jsonb then
    if not (b ? 'gross_area') then
      b:=b||jsonb_build_object('gross_area',(
        select gross_area from public.building_registers
        where register_pk=b->>'register_pk'));
    end if;
    r:=jsonb_set(r,'{building}',b);
    if b->'gross_area'='null'::jsonb and not exists (
      select 1 from jsonb_array_elements(r->'meta'->'missing_fields') item
      where item->>'path'='building.gross_area') then
      r:=jsonb_set(r,'{meta,missing_fields}',(r->'meta'->'missing_fields')||
        jsonb_build_array(jsonb_build_object('path','building.gross_area',
          'reason','register_gross_area_missing')));
    end if;
  end if;
  extra_ms:=extra_ms+extract(epoch from clock_timestamp()-lookup_started)*1000;
  r:=jsonb_set(r,'{meta,schema_version}','"1.3"');
