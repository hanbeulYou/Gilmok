...
- **S1 데이터 기반** (PR 2 인구·생활인구 적재 및 실제 R2 검증 완료, 리뷰 단계. S1 전체는 진행 중)
...
- R2 원본 6개 객체 게시 및 DuckDB 재읽기 검증 완료. 생활인구 411,512행 재집계 불일치 0. 원격 Supabase는 미전환이며 로컬 기본을 유지한다. 다음 PR은 새 세션에서 교통·상가 적재 계획부터 시작한다. 다음 작업 전 기준과 남은 확인 사항은 data-sources.md의 PR 2 인계 절을 읽는다.
