...
후속: [로그 원인·7단계 구조 변경·VACUUM·로컬 재개 검증·재시도 전 보고](s3-1-staged-restore-20260926.md). 아래는 실패 시점의 실행 이력이다.

2차 실행 상태: 사용자 재시도 승인 후 migration 29개 push 성공. 복원 도중 SSL 연결이 끊겨 중단했고, 재접속 읽기 전용 조회로 대상 18개 테이블이 모두 0행임을 확인했다. 아래 1차 기록은 이력으로 보존한다.
