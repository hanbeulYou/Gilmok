...
PR 2 최초 조사 시점의 중간 검증 기록이다. **아래 저장 형식 대기는 이후 사용자 결정으로 해소되었으며, JSONB는 거절되고 고정 컬럼을 채택했다. 최신 정책과 실측은 [후속 기록](pr2-living-fixed-20260919.md)을 따른다.** 원격 Supabase는 연결하지 않았다. 원본과 집계 Parquet는 로컬 준비 파일이며 R2 게시 완료를 뜻하지 않는다. 생활인구 최종 DB 저장 형식과 최신 행정동 경계는 미확정이므로 PR 2·S1 완료로 표시하지 않는다.
