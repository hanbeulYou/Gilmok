...
PR 2의 중간 검증 기록이다. 원격 Supabase는 연결하지 않았다. 원본과 집계 Parquet는 로컬 준비 파일이며 R2 게시 완료를 뜻하지 않는다. 생활인구 최종 DB 저장 형식과 최신 행정동 경계는 미확정이므로 PR 2·S1 완료로 표시하지 않는다.
...
현재 승인된 장형은 생활인구만으로 500MB 목표를 초과한다. JSONB 대안을 제안한 상태이며 이 PR의 현재 마이그레이션은 경계만 생성한다. `living_pop` 물리 스키마와 DB 적재는 답변을 기다린다.
