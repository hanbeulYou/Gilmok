...
### 원격 복원 실패 후 승인된 구조 변경

2026-09-26 사용자 요청: 단일 트랜잭션 복원을 경계·인구·교통·상가/학원/학교·건물·실거래/임대·기준 분포의 7개 독립 트랜잭션으로 바꾼다. 데이터와 private 단계 완료 기록(고정 manifest 해시·행수/digest·시간)을 함께 커밋하고 완료 단계는 검증 후 건너뛴다. 단계 내부 COPY는 50,000행 단위이며 TCP keepalive를 적용한다. 최초 29개 migration은 원격 적용 완료, 상태 테이블 migration 1개가 추가된다. 원본 manifest·적재 범위·정밀도는 유지한다.

[원인·로컬 검증·재시도 전 보고](../validation/s3-1-staged-restore-20260926.md): 원격 실패 원인은 WAL 공간 부족이다. 승인된 VACUUM 1회와 로컬 18테이블 digest·실패 후 재개 검증을 완료했다. 실측 디스크가 2.08GB여서 추가 디스크 변경은 사용자에게 별도 확인하고 원격 재시도 전 보고한다.

### 원격 실행 최신 상태 — 8GB 확장 후

사용자 승인으로 gp3 8GB 확장 및 원격 7단계 복원을 완료했다. 총 30개 migration, 18테이블 digest 일치, 유지보수 후 DB 889,810,067 byte. 6조합 HTTP 검증에서 anon role 3초 statement timeout(57014)으로 중단했고 Auth·Vault·webhook은 미실행이다. 원격 실행 승인은 유지되지만 실패 시 후속 단계 중단 조건에 따라 재실행하지 않았다. [원격 실측·남은 작업](../validation/s3-1-remote-20260926.md)을 따른다.
