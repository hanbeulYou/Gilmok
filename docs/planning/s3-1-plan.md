...
`ingest/measure_product_rpc.py`에서 기존 6조합을 동일 적용한다: 대치 `(37.494612,127.063642)`, 학여울 `(37.496663,127.070594)`, 한티 `(37.496237,127.052873)` × 500/1,000m, 2층. 새 세션 첫 실행 1회와 웜 30회를 분리하며 EXPLAIN ANALYZE 웜 p95 < 1,000ms가 기준이다. HTTP 왕복은 익명 로그인 세션(authenticated)으로 별도 기록한다. 응답의 측정 시각·실행 시간은 값 대조에서 분리한다.
