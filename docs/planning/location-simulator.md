...
## S2-1 진행 기록 — 2026-09-23

[채점 명세 v0.1.1](scoring-spec.md)의 첫 태스크인 기준 분포 배치를 구현하고 로컬 Supabase·R2 실제 검증을 마쳤다. 10,127셀×2반경×8지표=162,032행이며, 축별 모집단·결측 비율·실행 시간은 [검증 기록](../validation/s2-1-score-reference-20260923.md)에 있다. S2 전체는 진행 중이다. S2-1 PR 머지 후 main에서 S2-2 채점 순수 함수를 구현한다. 원격 전환 및 S2-3·S2-4는 이번 범위에 포함하지 않는다.

## S2-2 진행 기록 — 2026-09-23

S2-1(#12) 머지 후 채점 명세 v0.1.2에 따른 순수 함수·프리셋·ScoreResult v0.1, 입력 계약 v1.3의 gross_area, 정밀도 보존 reference 조회를 구현했다. 고정 3좌표×800/1000m×2층의 전체 결과와 역삼로460 3/4층 검증은 [S2-2 검증](../validation/s2-2-scoring-20260923.md)에 기록했다. 이 좌표들은 실제 학원 검증 대상이 아니므로 순위로 해석하지 않는다. S2 전체는 진행 중이며 가시성(S2-3)과 실제 학원 검증·임대료 보정(S2-4)은 별도 세션이다.

## S2-3 진행 기록 — 2026-09-23

GitHub #13·#14 머지 후 승인된 가시성 Worker와 5186 입력 RPC를 구현했다. 명세 v0.1.4, 역삼로460 3층 가시율 11.649874%, 역 동선20점 가시1/차폐10/제외9다. 순수 함수/실제 브라우저 Worker 결과 및 PostGIS 독립 검산이 일치했다. [현장 대조표](../validation/s2-3-visibility-field-report.md)와 [검증](../validation/s2-3-visibility-20260923.md)을 따른다. 현장 대조 및 S2-4 실제 학원 순위 검증은 아직 수행하지 않았다.

## S2-3 노출 조건 v0.2 후속 — 2026-09-24

PR #15 머지 후 사용자 승인으로 축을 exposure(건물 배치상 노출 조건)로 바꾸고 demand 30·exposure 5를 적용했다. 세부 규칙은 [명세 §5.5](scoring-spec.md) v0.2를 따른다. 1km 내 모든 역 각각20점, 링30/60/100m×36, 내부 샘플 최대30m 이동을 반영했다. 역삼로460 3층 가시 비율은12.460209%, 역 동선4/100점·링23/108점이 가시다. [현장 대조표](../validation/exposure-v02-field-report.md)와 [검증](../validation/exposure-v02-20260924.md)에 근거·시간을 기록했다. 가로수·가로시설물·간판 크기 미반영, 현장 확인 필요. S2-4는 별도 태스크다.
