...
> 작성일: 2026-09-16 · 버전: v1.4 (250m 격자 실파일 검증)
...
- 형태: 월별 ZIP의 CP949 CSV. 2026-06·07·08월 실제 파일을 모두 확인했다. 7월은 ZIP 안에 월별 디렉터리가 있고 6·8월은 CSV가 루트에 있다. 원본은 `raw/living_population/{YYYY-MM}.parquet`에 컬럼과 `*`를 보존한다. 이번 검증본은 로컬 준비 파일이며 R2 업로드 완료를 뜻하지 않는다.
...
- 3명 이하의 `*`는 NULL. 남녀 중 하나라도 비공개이면 해당 연령대 합계도 NULL. 집계 기간 중 비공개 조각이나 누락 날짜가 있으면 해당 셀·요일 유형·시간·연령 평균은 NULL로 남기고 `observed_days`, `expected_days`, `suppressed_parts`를 보존한다. 미관측 날짜·셀을 0으로 채우지 않는다.
...
- 적재 범위: 서울 전체 최근 완료 3개월. 월평균의 평균이 아닌 전체 기간의 날짜 수로 나눈다. 현재 2026-06-01~08-31 원본 23,344,369행, 관측 셀 8,598개를 확인했다.
...
- `living_pop`의 공간 키는 `tot_reg_cd` 대신 `(resolution_m, cell_id)`를 사용한다. 기존 장형 계약은 `living_pop(resolution_m, cell_id, dow_type, hour, age_band, avg_pop)`이며 관측·결측 수와 공통 출처를 추가한다. **저장 형식 변경은 미승인**: 실제 집계는 6,172,680행이고 PG 표본의 인덱스 포함 환산 용량은 약 1.37GB다. 동일 연령값·결측 정보를 JSONB map으로 보존하는 411,512행 구조는 약 353MB로 추산되어 제안했으며, 답변 전에는 `living_pop` 테이블을 생성하지 않는다. 500MB 목표 충족·전체 DB 용량·RPC 성능을 확인한 상태가 아니다.
...
- 행정동 경계 **확인 필요**: 공식 생활인구 뷰어의 2025-06-30 GeoJSON은 426개, OA-22160에서 2026-09-19 받은 SHP(EPSG:5181)는 425개다. 최신 인구 427개와 맞지 않는다. 특히 구 용신동을 신설동·용두동으로 임의 분할하지 않는다. 인구 CSV 정규화만 완료했고 현재 코드와 정확히 연결되는 경계 확보·DB 적재는 미완료다.
