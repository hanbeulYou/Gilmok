...
- 실제 파일 확인: `.local/downloads/AL_D010_11_20260909.zip`은 서울 전체 DBF 695,754행이다. 압축을 풀지 않고 읽었으며 PRJ는 **EPSG:5186**, 문자 속성은 CP949로 정상 해석했다. `A23='11680'`인 강남구 **28,228행**, PNU 유효 100%, 지상층수>0 **22,241행(78.79%)**, 원천 높이>0 **16,653행(58.99%)**다. 원천 29컬럼·완전 중복 1쌍·self-intersection 4건·비교 결과는 [검증 기록](../validation/pr5-footprints-20260920.md)을 따른다. 최종 정규화 처리와 적재는 PR 5 계획 승인 후 구현한다.
...
- SHP 운영은 사용자 결정에 따라 **수동 다운로드 후 R2 raw/에 게시, 갱신 주기 분기**다. 로그인 다운로드를 자동화하지 않는다. 수동 ZIP의 전체 원문 속성·원천 좌표 geometry를 Parquet로 보존하고, ZIP 해시·PRJ·컬럼 정의서·배포/행 기준일을 기록한다. 새 파일 없이 기준일을 새 분기로 올리지 않는다. 이번 조사에서는 아직 게시하지 않았다.
- 대장 호출 계획: 14개 법정동·일반/산 21조합에서 totalCount 합계 표제부 24,324행·층별개요 198,637행을 확인했다(1행씩 응답을 받은 건수 조사이며 전체 수집 전). 100행 페이지로 각각 255/2,000회, 합계 약 2,255회다. SHP의 21,753개 PNU를 각각 두 API에 요청하는 최소 43,506회 대신 동 단위 페이지 수집을 우선한다. 계정 잔량·실제 전체 적재 연결률은 구현 시 별도 검증한다.
- 적재 범위: S1은 강남구의 footprint·건축물대장·층별개요. 서울 전체 건축물 적재는 S2 초반 별도 태스크다.
- 확정 높이: SHP 또는 WFS 도형의 원천 높이>0이면 그대로 사용한다. 없으면 양수 지상층수×3.3m, 원천 주용도가 확인된 1층 단층 상업건물은 4.0m다. 이때 `height_source='floors_estimate'`, `estimated=true`, `height_estimated=true`다. 둘 다 없으면 `height_m=NULL`, `height_source='unknown'`으로 유지하고 3D 표시·차폐 입력에서만 4.0m를 사용한다. 원천 양수 높이는 `height_source='source'`다.
...
- PR 5 스키마 제안: 도형 `buildings`, 현행 대장 PK(text)의 `building_registers`, 원천 행을 보존하는 `building_floors`와 명시적 연결 관계로 나눈다. 도형·표제부를 일대일로 강제하지 않으며 PK 미연결 도형도 유지한다. 원천 ID·PNU·geometry·높이/층수·용도·출처와 추정/수리/연결 상태를 보존한다. 상세 필드와 마이그레이션은 [PR 5 계획](pr5-buildings-plan.md)의 승인 대상이다.
