...
1. **채점은 순수 함수다.** `score(inputs_primary, inputs_school, buildings, exposure, candidate, preset, reference, context) → ScoreResult`. 서버(Edge Function)와 클라이언트(가중치 슬라이더 재계산)가 `/lib/scoring`의 같은 코드를 쓴다. 외부 호출·DB 조회·Date.now 없음. reference는 반경·preset.id/reference_version/schema/source 버전이 일치하는 기준 분포다. context의 검증된 서울 경계 거리·법정동 코드표·computed_at(또는 inputs_primary.meta.computed_at)을 주입한다. 없는 보조 입력은 미확인으로 남긴다.
...
| `exposure_inputs_v021(lng, lat)`          | 노출 조건 레이캐스트: 건물1,230m·역1,200m·학교1,000m                                           |
...
**가중치 근거(가설):** 학원은 목적지형 업종이라 지나가다 들어오는 유동보다 통원권 안의 학령인구가 결정적이다(demand 30). 대치동처럼 밀집이 곧 목인 업종 특성을 cluster 15로 둔다. 과밀은 점수가 아니라 포화 지표(§4.4)로 따로 보여준다. 간판보다 검색·소개로 오는 업종이라 건물 앞 도로·맞은편에서의 간판 노출(exposure) 5. **이 순서 자체가 §8 검증 대상이다.**
...
**점수 샘플:** 5186 격자 북쪽0°에서10° 간격36방향 × 거리 **{20,40,60}m =108점**. 각 가중치100/d다. 100m 이상 링과 역·학교 동선은 점수에서 제외한다. visible_ratio는 **유효 링의 가시 가중치/유효 링 전체 가중치**이고 점수는100×visible_ratio다. 역·학교 유무·차폐·자료 결측은 이 비율을 바꾸지 않는다. 건물 자료 결측 또는 유효 링 가중치0이면 점수NULL이다.

**역·학교 근거(점수 미반영):** 기존 후보→역20등분 i/20(i=1…20), 후보→학교10등분 i/10(i=1…10) 샘플로 차폐를 확인한다. 각 역/학교에서 후보 쪽으로 올 때 처음 보이는 **유효 가시 샘플까지의 거리(m)** 하나를 `first_exposure_distance_m`로 반환한다. 출발점과 후보 사이 선분 안에 있는 이동 후 샘플 중 출발점까지 거리가 최소인 것을 선택한다. 출발점 바깥으로 밀린 샘플은 이 거리 후보에서 제외한다. 출발점 자체가 가시면0m, 가시 샘플이 없거나 해당 소스가 결측이면NULL과 사유를 반환한다. 역/학교는 각각 한 값이며 서로 평균하거나 점수에 가산하지 않는다.

이 거리는 EPSG:5186 직선 대표점 동선의 **샘플 기준 추정값(estimated=true)** 이다. 역20등분/학교10등분 간격과 최대30m 이동 때문에 실제 최초 노출 경계·보행 거리·역 출입구까지의 거리가 아니다. 근거에는 샘플 간격·선택 샘플 ID를 함께 남긴다. 샘플 분류·좌표는 검산용으로 보존하지만 사용자 대조표는 역/학교당 단일 거리로 표시한다.

**footprint 내부·경계 샘플 처리:** 후보에서 원래 샘플을 향하는 방향을 유지해 그 샘플 위치부터 바깥쪽으로 최대30m 밀어낸다. 모든 도형 합집합을 벗어난 첫 구간을 사용한다. 붙거나 겹친 건물은 함께 통과하며 구멍 내부는 건물 밖이다. 첫 경계에서 최대1mm 밖(더 좁은 빈 구간은 중간점)을 사용한다. 30m 안에 못 나오거나 원점과 같아 방향을 정할 수 없으면 제외한다. 이미 밖이면 이동하지 않는다. 링 가중치는 **원래 반경 기준**이며 도로에 스냅하지 않는다.

원래/최종 좌표·이동거리·이동 여부와 전체·집합별 제외 비율(제외 점 수/생성 점 수)을 남긴다. 점수 근거에는 링의 제외 전/후 가중치와 제외 가중치 비율을 기록한다. 역·학교 샘플의 점수 가중치는0이다. 차폐는 footprint와0~높이의 수직 프리즘, 시선 선분의 교차로 판정하며 후보 ID 전체만 제외한다. WFS·작은 부속/창고·unknown4m도 차폐에 포함한다. 건물·가시 판정은 바운딩박스 필터 후 순수 함수로 계산하며 Worker와 Node가 같은 함수를 호출한다.

**현장 진술 대조:** 사용자 진술 “맞은편에서 보임”에 대해 링108점 중 가시점이54점 이상(가중치 미적용)이면 명시한 기준과 일치로 기록한다. 제외점도 생성108점 분모에 남겨 과대해석을 피한다. 실제 맞은편 방향·간판 확인이나 §8의 전체 모델 검증을 완료했다는 뜻은 아니다. 2026-09-26 사용자 확인:30·40·60m는 오타이며 점수와 현장 판정 모두20·40·60m의 같은 링을 사용한다.

**필수 한계 문구:** 근거에는 계산 성공·결측·대기와 무관하게 항상 **“가로수·가로시설물·간판 크기 미반영, 현장 확인 필요”** 를 표시한다. 지형·실제 보도·간판 크기·가로수 등을 이 모델로 추정하지 않는다.
...
- §5.5 전체 개정: 1km 모든 역, 30/60/100m 링, 바깥 방향 최대30m 이동·제외 비율 근거. 축 key는 exposure, 표시는 건물 앞 도로·맞은편에서의 간판 노출이다.
