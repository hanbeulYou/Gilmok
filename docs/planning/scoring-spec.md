...
> 상태: **v0.3, 대치동 3곳 검증 통과(b>a>c)** — 2026-09-26 사용자 S2 마감 승인, PR #18 머지. 검증 범위·조정 이력·과적합 유의 사항은 §8을 따른다.
...
S2-1~S2-4를 마감했다. PR #18에서 승인된 두 버그를 수정하고 cluster 스케일을 1회 조정했다. 다른 축·가중치는 유지하며, 보류 과제와 S3 인계는 [기획서](location-simulator.md#s3-인계--s2-마감)를 따른다.
...
## 3. 축과 가중치 — academy_v0 v0.3 (대치동3곳 검증)
...
**가중치 근거(가설):** 학원은 목적지형 업종이라 지나가다 들어오는 유동보다 통원권 안의 학령인구가 결정적이다(demand 30). 대치동처럼 밀집이 곧 목인 업종 특성을 cluster 15로 둔다. 과밀은 점수가 아니라 포화 지표(§4.4)로 따로 보여준다. 간판보다 검색·소개로 오는 업종이라 건물 앞 도로·맞은편에서의 간판 노출(exposure) 5. 이번 통과 범위와 미검증 항목은 §8을 따른다.
...
| `meta.height_quality` 부속 제외 후 unknown_ratio > 0.3                  | −10                     | "주변 건물 ○%가 높이 미상, 노출 조건 신뢰 낮음"  |
| `meta.building_lookup.status ∈ {pending, processing}`                   | −15                     | "건축물대장 조회 대기 중"                     |
| building 대장 미연결(`building.location_basis='footprint'`·`building.register_pk=NULL`, pending 아님) | −10                     | "건물 대장 미연결, 용도·승강기 미확인"     |
| `transit.subway_units_missing_golden > 0` | −5 | "지하철 일부 시간대 데이터 없음" |
| 후보가 서울 경계 1km 이내                                               | −5                      | "경기 정류장 데이터 없음"                     |
| rent 사용자 입력 없음                                                   | (결측 축 감점으로 처리) |                                               |

신뢰도 실제 문자열과 결측 사유 코드 전체 목록은 [S3 인계](location-simulator.md#신뢰도-사유-전체-목록--현행-코드-기준)에 있다.

---

## 8. 검증 절차 — 대치동 실제 학원 3곳

**v0.3 대치동 3곳 검증 통과(b>a>c), 2026-09-26 사용자 승인으로 S2 마감.** 아래 절차와 사전 순위를 보존하며, 이번 검증의 범위를 §8.4~§8.6에 기록한다.
...
- **environment 가설(숙박 감점)**이 대치동 안에서 의미 있는 차이를 만드는지. 미확인 사항은 후속 검토로 남기며 이번 마감에서는 축을 제거하지 않는다.
...
### 8.4 검증 결과 — 2026-09-26

주소→PNU→단일 도형 포함 확인 경로, 반경800m, 실제 exposure Worker 0.2.2, 임대료·전용면적 미입력으로 채점했다. 명세/프리셋0.3, 입력1.3, raw reference0.1.2(snapshot `20260923T111436Z`)다. [조정 전](../validation/s2-4-20260926.md)과 [v0.3 전체 점수·근거](../validation/s2-4-v03-20260926.md), [기계 판독 결과](../validation/s2-4-v03-20260926.json)를 보존한다.

| 후보 | 주소·층 | cluster | 총점 | 신뢰도 | 순위 |
|---|---|---:|---:|---:|---:|
| a | 역삼로460 3층 | 97.477406 | 85.085005 | 90 | 2 |
| b | 도곡로409 2층 | 98.410036 | 93.372948 | 90 | 1 |
| c | 역삼로546 덕일빌딩3층 | 85.910144 | 83.863983 | 85 | 3 |

사전 순위 **b>a>c**, 조정 전 **b>c>a**, 최종 **b>a>c**. 시드20260926의 동일 서울250m 격자5셀도 재채점했다. 총점NULL 0/5, 신뢰도30 미만0/5, p50 이하2셀에서 cluster=0이었다. 위치 교체·재추출은 하지 않았다. Worker/Node 결과8곳 일치, lint·typecheck·test·DB150건 및 PR #18 CI 통과 기록을 따른다.

### 8.5 조정 이력 — cluster 스케일 1회

1. 조정 전에 두 승인 버그를 수정했다. 건물 미적재5셀의 exposure100→NULL, b의 요청 층10003 학원 인식으로 building75→100. 이는 프리셋 조정 횟수와 구분한다.
2. cluster의 raw=ln(1+n)과 가중치15를 유지하고 서울 백분위 점수를 §4.4 고정 선형 스케일로 한 번 교체했다. `score=clamp((raw−p50)/(upper−p50)×100,0,100)`, p50=3.332204510175204, upper=p99.97=7.070653980704802. 800m cluster 비NULL9,694개 분포의 scalar percentile_cont를 사용했다. p99부터0.01%p 간격으로 탐색하여 세 곳 모두100 미만인 최초 상한을 선택했다.
3. 앞서 논의한 p50/p95·50점 시작식은 세 곳 모두100이 되어 채택하지 않았다. 사용자 재지정 후 위 식만 최종 프리셋으로 적용했다. 다른 축·가중치·포화 임계값·반경은 유지했다. 조정 전→버그 수정 후→v0.3 결과는 검증 문서에서 구분한다.

### 8.6 과적합 유의와 검증 범위

**세 곳을 보고 상한 분위수를 선택했으므로 이 세 곳의 순위 일치는 독립적인 일반화 검증이 아니다.** 대치동3곳과 고정 무작위5셀의 점검 통과를 서울 전체·다른 업종·향후 창업 성과의 검증으로 확대하지 않는다. 무작위5셀은 이상값 점검이며 정답 순위가 있는 별도 평가 집합이 아니다. 후속 프리셋은 별도 후보·지역의 사전 판단과 비교하고 한 번에 하나씩 조정한다.

§8.3의 축별 직관 판정 전체, 500m 비교, 임대료 lo/hi 보정은 이번 완료 범위에 포함하지 않는다. 역삼로460 exposure의 현장 진술 판정(가시32/108점, 절반 기준 미충족)은 기존 기록 그대로이며, 총점 순위 통과로 현장 노출까지 검증 완료한 것으로 바꾸지 않는다. S2 마감 이후 보류 항목은 기획서의 S3 인계 절을 따른다.
...
| 태스크              | 내용                                                                                                                                   | 완료 기준                                    |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| S2-1 기준 분포 — 완료 | score_reference 배치·월간 갱신·정밀도 보존 조회 | 전수 대조 및 실제 검증 기록 완료 |
| S2-2 채점 함수 — 완료 | /lib/scoring 순수 함수, 프리셋, 최초 ScoreResult v0.1 → 현행 v0.3 | 결측·재배분·규칙·실제 입력 검증 완료 |
| S2-3 노출 워커 — 완료 | 순수 레이캐스트·Web Worker, 현행 모델0.2.2 | 합성 fixture·실제 좌표·계산 시간·Node 동등성 검증 |
| S2-4 검증 — 완료 | 3곳+고정 무작위5셀, cluster 스케일1회 조정, v0.3 | b>a>c, 2026-09-26 사용자 S2 마감 승인 |
| S2-5 서울 전체 건물 — 이월 | 원격 용량 실측·Pro 전환 검토 후 별도 적재 | 현재 강남구만 적재; S2 완료 항목과 구분 |
...
PR #18 머지 후 사용자 승인으로 상태를 v0.3 대치동3곳 검증 통과로 변경했다. 수식·코드 변경 없이 §8 결과·조정1회·과적합 유의 사항 및 기획서 S3 인계를 문서화했다.
